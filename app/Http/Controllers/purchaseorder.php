...
class purchaseorder extends Controller
{
    public function __construct()
    {
        ...
        if(session::get('loggindata.loggedinuser.usertypeRole') == 'Admin')
        {
            ...
            {
                $allstores= session::get('loggindata.loggeduserstore');
            }
            else
            {
                $allstores = store::get();
    ...
    {   
        ...
        else
        {
            ...
            if(session::get('pofilterdata.store')!='')
            {
                $store[]= session::get('pofilterdata.store');
            }
            else if(count(session::get('loggindata.loggeduserstore'))=='0')
            {
                $store = session::get('loggindata.loggeduserstore');
            }
            else
            {
            ...
            $allpurchaseorder1=productpurchaseorder::where('productpurchaseorder.poprocessstatus', 'LIKE', '%'.$processstatus.'%')
            ...
            if(count($store) > 0)
            {
                $allpurchaseorder1->whereIn('productpurchaseorder.storeID', $store);
            }
            $allpurchaseorder =$allpurchaseorder1->get(array(
            ...
            $allincompletepurchaseorder1=productpurchaseorder::where('poprocessstatus', '0')
            ...
            if(count($store) > 0)
            {
                ...
                $allincompletepurchaseorder1->whereIn('productpurchaseorder.storeID', $store);
            }
            $allincompletepurchaseorder =$allincompletepurchaseorder1->get(array(
                'productpurchaseorder.poID',
                'productpurchaseorder.ponumber',
                'productpurchaseorder.porefrencenumber',
                'productpurchaseorder.storeID',
            ...
            $poviewdata = ['allpurchaseorder'=>$allpurchaseorder, 'firstday'=>$firstday, 'lastday'=>$lastday, 'processstatus'=>$processstatus, 'supplier'=>$supplier, 'store'=>$store, 'allincompletepurchaseorder'=>$allincompletepurchaseorder];
    ...
    {   
        ...
        else
        {
            ...
            if($checkpoitem > 0)
            {
                ...
                $updatefindpo->userID = session::get('loggindata.loggedinuser.id');
                $updatefindpo->created_at = Carbon::now()->toDateTimeString();
                $updatefindpo->save();
    ...
    {   
        ...
        else
        {
            ...
            $allpurchaseorder1=productpurchaseorder::where('poprocessstatus', '!=', '0')
            ...
            $stockinhand1=productpurchaseorder::where('poprocessstatus', '1')->join('productpurchaseorderitem', 'productpurchaseorderitem.ponumber','=', 'productpurchaseorder.ponumber')
            ->join('products', 'products.productID', '=', 'productpurchaseorderitem.productID')
            ->leftJoin('masterbrand', 'masterbrand.brandID', '=', 'products.brand')
            ->leftJoin('mastercolour', 'mastercolour.colourID', '=', 'products.colour')
            ->leftJoin('mastermodel', 'mastermodel.modelID', '=', 'products.model')
            ...
            if(count($store) > 0)
            {
                $stockinhand1->where('productpurchaseorder.storeID', $store);
            }
            $stockinhand = $stockinhand1->get(array('productpurchaseorderitem.productID', 'productpurchaseorderitem.poquantity', 'productpurchaseorderitem.receivequantity', 'products.productname', 'masterbrand.brandname', 'mastercolour.colourname', 'mastermodel.modelname', 'productpurchaseorder.ponumber','productpurchaseorder.storeID'));
    ...
    {   
        ...
        else
        {
            ...
            $allpurchaseorderitem1=productpurchaseorder::where('productpurchaseorder.ponumber', $id)
            ->join('productpurchaseorderitem', 'productpurchaseorderitem.ponumber', '=', 'productpurchaseorder.ponumber')
            ->join('products', 'products.productID', '=', 'productpurchaseorderitem.productID')
            ->leftJoin('masterbrand', 'masterbrand.brandID', '=', 'products.brand')
            ->leftJoin('mastercolour', 'mastercolour.colourID', '=', 'products.colour')
            ...
            if(count($storeID) > 0)
            {
                $allpurchaseorderitem1->where('productpurchaseorder.storeID', $storeID);
            }
            $allpurchaseorderitem = $allpurchaseorderitem1->get();
    ...
    { 
        ...
        { 
            ...
            {
                if(count(session::get('loggindata.loggeduserstore')) > 2)
                {
                ...
                else
                {
                    ...
                    {
                        if($storeid==$request->input('storeid'))
                        {
    ...
    {   
        ...
        else
        {
            ...
            $alldrreceiveorder1=demoreceiveorder::whereIn('receiveorderstatus', ['0', '1', '2'])
            ->join('store', 'store.store_id','=','storeID')
            ->join('users', 'users.id','=','userID')
            ->with('demoitem')
            ->with('demosupplier');
            if(count($store) > 0)
            {
                $alldrreceiveorder1->whereIn('demoreceive.storeID', $store);
            }
            $alldrreceiveorder = $alldrreceiveorder1->get(
                array(
                    'demoreceive.receiveID',
                    'demoreceive.receiveInvoiceID',
                    'demoreceive.supplierID',
                    'demoreceive.storeID',
                    'demoreceive.referenceNumber',
                    'demoreceive.receiveorderstatus',
                    'demoreceive.userID',
                    'demoreceive.created_at',
                    'store.store_name',
                    'users.name'
                )
            );
    ...
    {   
        ...
        else
        {
            ...
            $alldrorderitem1=demoreceiveorder::leftJoin('demoreceiveorderitem', 'demoreceiveorderitem.receiveInvoiceID', '=', 'demoreceive.receiveInvoiceID')
            ->leftJoin('products', 'products.productID', '=', 'demoreceiveorderitem.productID')
            ->where('demoreceive.receiveInvoiceID', $id);
            if(count($store) > 0)
            {
                $alldrorderitem1->whereIn('storeID', $store);
            }
            $alldrorderitem =$alldrorderitem1->get();
