...
class stockreturnController extends Controller
{
    ...
    {   
        ...
        else
        {
        	...
        	if(session::get('loggindata.loggeduserstore')=='')
        	{
        		$getstockreturn = stockreturn::leftJoin('store', 'store.store_id', '=', 'storeID')
	        	->leftJoin('mastersupplier', 'mastersupplier.supplierID', '=', 'stockreturn.supplierID')
	        	->where('stockreturn.storeID', 'LIKE', '%'.$storeID.'%')
	        	->whereBetween('stockreturn.stockreturnDate', [$firstday, $lastday])
	        	->where('mastersupplier.supplierID', 'LIKE', '%'.$supplierID.'%')
	        	->where('stockreturn.stockreturnStatus', 'LIKE', '%'.$returnstatus.'%')
	        	->get();
        	}
        	else
        	{
	        	...
	        	->where('stockreturn.storeID', 'LIKE', '%'.$storeID.'%')
	        	->whereBetween('stockreturn.stockreturnDate', [$firstday, $lastday])
	        	->where('stockreturn.supplierID', 'LIKE', '%'.$supplierID.'%')
	        	->where('stockreturn.stockreturnStatus', 'LIKE', '%'.$returnstatus.'%')
	        	->get();
        	}
