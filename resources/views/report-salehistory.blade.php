                                ...
                                <table id="datatable" class="table table-bordered" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                    ...
                                    @foreach($allsale as $salehistory)
                                        <tr>
                                            ...
                                            <td>
                                                @foreach($salehistory->orderitem as $orderitem)
                                                    @if($orderitem->stockID !="")
                                                        {{App\productstock::where('psID', $orderitem->stockID)->pluck('productimei')->first()}}
                                                    @endif
                                                @endforeach
                                            </td>
                                    ...
                                    @endforeach
                                    @foreach($refundsale as $refundsale)
                                        <tr style="background-color: #f1525247;">
                                            <td>
                                                @php
                                                echo date('d-m-Y H:i:s', strtotime($refundsale->created_at))
                                                @endphp
                                            </td>
                                            <td><a href="refundinvoice/{{$refundsale->refundInvoiceID}}" style="color: #007bff;"> {{$refundsale->refundInvoiceID}} </a></td>
                                            <td>{{$refundsale->customer['customerfirstname']}} {{$refundsale->customer['customerlastname']}}</td>
                                            <td>
                                                @foreach($refundsale->orderitem as $orderitem)
                                                    @if($orderitem->stockID !="")
                                                        {{App\productstock::where('psID', $orderitem->stockID)->pluck('productimei')->first()}}
                                                    @endif
                                                @endforeach
                                            </td>
                                            <td>{{$refundsale->name}}</td>
                                            <td>{{$refundsale->orderType}}</td>
                                            <td>
                                                {{$refundsale->orderpayment()->sum('paidAmount')}}
                                            </td>
                                            <td>{{$refundsale->store_name}}</td>
                                            <td>
                                                @if($refundsale->refundStatus == 0)
                                                <span class="badge badge-danger">In-complete</span>
                                                @else
                                                <span class="badge badge-success">Complete</span>
                                                @endif
